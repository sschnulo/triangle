#
# =============================================================================
#  PYTHON SCRIPT FOR PLOTTING TURBOMACHINERY VELOCITY DIAGRAMS
# =============================================================================
...
# list of blade rows in the model


# dictionary of blade row results
#   'bladerowName'
#
#   'VelocityIn'
#   'alphaIn'
...
# function to generate a set of x,y points given a length and an angle
#    start from the origin, offset is handled in plot function


# function that plots the velocity triangles and blade lines
#    entrance absolute
#    entrance relative (if rotor)
#    entrance blade velocity (if rotor)
#
#    exit absolute
#    exit relative (if rotor)
#    exit blade velocity (if rotor)
#
#    entrance blade metal
#    exit blade metal
...
def plotVelocityTriangles(offsetx, offsety):
    '''Plots turbomachinery velocity diagrams and blade cartoons.'''

 
    ax1 = pylab.subplot2grid((1,4),(0,0))
    ax1.axis([ 0., 1000, -2000., 2000. ])
    # entrance absolute (positive alpha is -y direction)
    dx =  BR ['velocityIn'] * numpy.cos( BR ['alphaIn'] )
    dy = -BR ['velocityIn'] * numpy.sin( BR ['alphaIn'] )
    ax1.arrow( offsetx, offsety, dx, dy, width=6, head_width=40,
                 length_includes_head='true', color='green' )


    # entrance relative
    dx =  BR ['vRelIn'] * numpy.cos( BR ['betaIn'] )
    dy =  BR ['vRelIn'] * numpy.sin( BR ['betaIn'] )
    ...
    pylab.subplot2grid((1,4),(0,1),colspan=2)

    if BR ['bladerowName'] == 'vane1' or BR ['bladerowName'] == 'vane2' or BR ['bladerowName'] == 'vane3' or BR ['bladerowName'] == 'vane4':
        master( 0.16, 1000., -34.*numpy.pi/180., offsetx, offsety, 'convex' )

    print "last x = ", lastx, "    last y = ", lasty


    if BR ['bladerowName'] == 'rotor1' or BR ['bladerowName'] == 'rotor2' or BR ['bladerowName'] == 'rotor3' or BR ['bladerowName'] == 'rotor4':
        master( 1.18, 1000., -35.*numpy.pi/180., offsetx,-offsety, 'concave' )


    '''
    # blade front half, offset 15% from relative
    offsetx = 100 + 1.15*( BR ['vRelIn'] * numpy.cos( BR ['betaIn'] ) )
    offsety = 200 + 1.15*( BR ['vRelIn'] * numpy.sin( BR ['betaIn'] ) )
    ...
    ax3 = pylab.subplot2grid((1,4),(0,3))
    ax3.axis([ 0., 1000, -2000., 2000. ])
    # exit absolute (positive alpha is -y direction)
    offsetx = 0 
    offsety = 0
    dx =  BR ['velocityOut'] * numpy.cos( BR ['alphaOut'] )
    dy = -BR ['velocityOut'] * numpy.sin( BR ['alphaOut'] )
    ax3.arrow( offsetx, offsety, dx, dy, width=6, head_width=40,
                 length_includes_head='true', color='red' )


    # exit relative
    dx =  BR ['vRelOut'] * numpy.cos( BR ['betaOut'] )
    dy =  BR ['vRelOut'] * numpy.sin( BR ['betaOut'] )
    ax3.arrow( offsetx, offsety, dx, dy, width=6, head_width=40,
                 length_includes_head='true', color='green' )
...
for BR in BRnames:
   if BR ['bladerowName'] == 'vane1':# or BR ['bladerowName'] == 'rotor1':#or BR ['bladerowName'] == 'vane2' or BR ['bladerowName'] == 'rotor2': #or BR ['bladerowName'] == 'vane3' or BR ['bladerowName'] == 'rotor3' or BR ['bladerowName'] == 'vane4' or BR ['bladerowName'] == 'rotor4':
       #pylab.figure( figsize=( 10, 10) )
       # axes = [ 0., 1000., -2000., 2000. ]
      # #axes = [ 0., 2000., -1250.,  750. ]     # test_AMlossModel_blades
      # #axes = [-500., 3000., -1500., 2000. ]  # test_fanOD_blades
       # pylab.axis( axes )
       # pylab.linewidth = 10
       ...
       plotVelocityTriangles(offsetx, offsety)


      # offsetx = offsetx + 2000
      # offsety = offsety + ((-1)**n)*100
      # n = n+1
